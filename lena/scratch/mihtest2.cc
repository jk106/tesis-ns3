...
(wifiApNode.Get(0) -> GetObject<ConstantPositionMobilityModel>()) -> SetPosition(Vector(00.0, 0.0, 0.0));
  ...
  positionAlloc->Add (Vector (600.0, 0.0, 0.0)); //STA
  ...
  cvm->SetVelocity(Vector (50, 0, 0)); //move to left to right 10.0m/s
  ...
  Ptr<Ipv4StaticRouting> remoteHostStaticRouting = ipv4RoutingHelper.GetStaticRouting (p2pNodes.Get(1)->GetObject<Ipv4> ());
...
/**
  ...
  serverApps3 = udpServer3.Install (bsNodes1.Get(0));
  serverApps3.Start (Seconds (6));
  serverApps3.Stop (Seconds (duration));

  udpClient3 = UdpClientHelper (Ipv4Address("7.0.0.1"), 100);
...
UdpServerHelper udpServer2;
  ...
  Simulator::Schedule (Seconds (10), &setNewDevice, ssNodes.Get(0), 3);
  //Simulator::Schedule (Seconds (70), &setNewDevice, ssNodes.Get(0), 3);
