...
int main (int argc, char *argv[])
{
  ...
  std::vector<Ptr<Node> > nodesWifi1,nodesWimax,nodesLte;
  std::vector<int > indexWifi1_down,indexWimax_down,indexLte_down;
...
remoteHostStaticRouting = ipv4RoutingHelper.GetStaticRouting (p2pNodes.Get(0)->GetObject<Ipv4> ());
  ...
  Ptr<OutputStreamWrapper> routingStream= Create<OutputStreamWrapper>("dynamic9.routes",std::ios::out);
...
Ptr<FlowMonitor> monitor4;
  ...
  Simulator::Run ();
  monitor->SerializeToXmlFile("result9-0.xml",true,true);
  monitor1->SerializeToXmlFile("result9-1.xml",true,true);
  monitor2->SerializeToXmlFile("result9-2.xml",true,true);
  monitor3->SerializeToXmlFile("result9-3.xml",true,true);
  monitor4->SerializeToXmlFile("result9-4.xml",true,true);
