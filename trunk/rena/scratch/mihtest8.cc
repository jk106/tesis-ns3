...
remoteHostStaticRouting = ipv4RoutingHelper.GetStaticRouting (wifiApNodes.Get(1)->GetObject<Ipv4> ());
  ...
  Ptr<OutputStreamWrapper> routingStream= Create<OutputStreamWrapper>("dynamic8.routes",std::ios::out);
...
Ptr<FlowMonitor> monitor4;
  ...
  Simulator::Run ();
  monitor->SerializeToXmlFile("result8-0.xml",true,true);
  monitor1->SerializeToXmlFile("result8-1.xml",true,true);
  monitor2->SerializeToXmlFile("result8-2.xml",true,true);
  monitor3->SerializeToXmlFile("result8-3.xml",true,true);
  monitor4->SerializeToXmlFile("result8-4.xml",true,true);
